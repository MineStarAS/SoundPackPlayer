...
class SoundListGUI(val soundPlayer: SoundPlayer, val backGUI: GUI) : GUI {
    override val player = soundPlayer.player
    override val pl = Main.pl
    override val gui = Bukkit.createInventory(null, 9 * 6, "[곡 목록]")
    ...
    override fun displaying() {
        ...
        val list = DataClass.soundDataList().toList()
    ...
    override fun clickGUI(e: InventoryClickEvent) {
        ...
        when (item) {
            ...
            nextPage -> {
                if (DataClass.soundDataList().toList().lastIndex < (page + 1) * 45) return
                ++page
            }
            back -> return backGUI.openGUI()
                ...
                if (e.click == ClickType.SHIFT_LEFT) soundPlayer.playListAdd(soundData)
            }
        }
        displaying()
    }
